<template>
  <div>
    ...
    <el-dialog v-drag title="滞留期预警设置" :visible.sync="dialogVisible" width="50%" class="showAll_dialog">
    ...
    >
      ...
      <el-table-column label="预警提前期(天)" align="center" :show-overflow-tooltip="true" prop="retentionLeadDay" />
      <!-- <el-table-column label="是否开启预警" align="center" :show-overflow-tooltip="true" prop="retentionAlarmEnabled" :formatter="alarmFormat" /> -->
      <el-table-column label="预警状态" align="center" :show-overflow-tooltip="true" prop="isRetentionAlarm">
        <template slot-scope="scope">
          <span v-if="scope.row.isRetentionAlarm=== 0"><el-tag type="success" size="small">正常</el-tag></span>
          <span v-if="scope.row.isRetentionAlarm=== 1"><el-tag type="warning" size="small">预警</el-tag></span>
      ...
      >
        <template slot-scope="scope">
          <el-button-group>
            ...
            <!-- <el-button v-if="scope.row.retentionAlarmEnabled === 1" size="small" type="danger" icon="el-icon-turn-off" @click="closeWarning(scope.row)">关闭预警</el-button> -->
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
...
import { searchInventoryBalanceSelective, alarmByRetentionDays, updateAlarm, exportRetentionAlarmExcel, searchInventoryBalanceNum } from '@/api/user'
...
export default {
  ...
  methods: {
    ...
    alarmFormat(row) {
      if (row.retentionAlarmEnabled === 1) {
        return '已开启'
      } else if (row.retentionAlarmEnabled === 0) {
        return '未开启'
      }
    },
    refresh() {
    ...
    submit() {
      this.$confirm('确定开启预警吗?', '提示', {
        ...
        updateAlarm({
          id: this.id,
          retentionAlarmEnabled: 1,
          retentionLeadDay: this.warningNum2
        }).then(res => {
          if (res.code === 200) {
    ...
    outExcel() {
      exportRetentionAlarmExcel({ searchDTO: {
