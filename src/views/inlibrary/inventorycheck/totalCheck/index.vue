<template>
  <div>
    ...
    <div class="btngroups">
      ...
      <el-button size="mini" type="success" icon="el-icon-standardInventoryByRange" @click="totalCheck">全库盘点</el-button>
      <el-button size="small" type="danger" icon="el-icon-refresh" @click="refresh">刷新</el-button>
      <!-- <el-button size="mini" type="warning" icon="el-icon-download" @click="outExcel">导出</el-button> -->
    </div>
    <el-dialog
      width="50%"
      class="stock_code"
      title="全库盘点"
      append-to-body
      :visible.sync="allCheckDialogVisible"
    >
      <p class="fast-tip">
        <i class="el-icon-question" />
        您选择了“全库盘点”，将盘点此库房所有物品的库存数量。
      </p>
      <span color="#fff">盘点编号：</span><el-input v-model="inventoryCode" size="small" disabled style="width: 160px;" />
...
import { searchStockInventorySelective, searchStockInventoryNum, stockInventoryUnAudit, getStockInventoryHeadCode, stockInventoryAudit, fullInventory, deleteStockInventoryAndDetail, exportStockInventoryAndDetailExcel } from '@/api/user'
...
export default {
  ...
  methods: {
    totalCheck() {
      this.allCheckDialogVisible = true
      this.getHeadCode()
    },
    getHeadCode() {
      getStockInventoryHeadCode({ code: 'INV' }).then(res => {
        if (res.code === 200) {
          this.inventoryCode = res.data
        }
      })
    },
    ...
    async startFullCheck() {
      ...
      if (res.code === 200) {
        this.$message.success('已成功生成全库盘点任务！请在页面查看详细内容')
        this.refresh()
      }
    },
    scanResult(row) {
