<template>
  <div>
    <el-row>
        ...
        <el-row>
          <div id="btnAndSearch">
              ...
              <div class="search">
                <el-form :inline="true" :model="formSearch">
                  ...
                  <el-form-item label="收货地址">
                    ...
                    <el-select v-model="formSearch.site" filterable clearable size="small" placeholder="请选择区县" @change="selectAreaFun($event)">
            ...
            <div class="btngroups">
              ...
              <el-tooltip class="item" effect="dark" content="手动填写toC客户订单" placement="bottom-start">
              ...
              <el-tooltip class="item" effect="dark" content="查看全部/未发运的toC客户订单" placement="bottom-start">
        ...
        <el-card :body-style="{padding:'0px'}" shadow="always">
          ...
          >
            ...
            <el-table-column :label="labelTitle" align="center">
              ...
              <el-table-column sortable="custom" label="订单创建时间" align="center" :show-overflow-tooltip="true" prop="placeTime">
                <template slot-scope="scope">{{ scope.row.placeTime| dateYMDHMSFormat }}</template>
              </el-table-column>
      ...
      <el-col v-show="showRight" :span="spanRight" class="rightPart">
        ...
        <div id="btnRight">
          <div class="btngroupsRight">
            <el-button-group>
              <el-tooltip class="item" effect="dark" content="返回toC客户订单页面" placement="bottom-start">
            ...
            <el-tooltip class="item" effect="dark" content="显示左侧订单列表" placement="bottom-start">
              ...
              <el-tooltip v-if="isDetailTwo" class="item" effect="dark" content="点击将此toC客户订单生成发运订单" placement="bottom-start">
                <el-button icon="el-icon-s-order" type="primary" @click="toDespatch">生成发运订单</el-button>
              </el-tooltip>
              <el-tooltip v-if="isDetail" class="item" effect="dark" content="点击修改此toC客户订单" placement="bottom-start">
                <el-button icon="el-icon-edit" type="primary" @click="editStart">修改</el-button>
              </el-tooltip>
              ...
              <el-tooltip v-if="isEdit" class="item" effect="dark" content="点击提交此toC客户订单" placement="bottom-start">
                <el-button icon="el-icon-document-checked" type="primary" @click="submitEdit">提交修改</el-button>
              </el-tooltip>
              <!-- <el-tooltip class="item" effect="dark" content="点击审核此toC客户订单" placement="bottom-start">
              <el-button icon="el-icon-receive" size="small" type="primary" @click="finish(scope.row)">审核</el-button>
            </el-tooltip> -->
              <el-tooltip v-if="isDetail" class="item" effect="dark" content="点击删除此toC客户订单" placement="bottom-start">
                <el-button icon="el-icon-delete" type="danger" @click="deledeNowAll">删除</el-button>
              </el-tooltip>
              <el-tooltip class="item" effect="dark" content="点击此toC客户订单生成的发运订单" placement="bottom-start">
          ...
          <el-card class="show_card1" :body-style="{padding:'0px'}">
            <div slot="header" align="center" style="padding: 15px">
              <span class="title">客户订单-toC 单头信息</span>
            </div>
        ...
        <el-card style="background-color: #081b26c3;" :body-style="{padding:'0px'}">
          <div slot="header" style="padding: 15px" align="center">
            <span class="title">客户订单-toC 商品信息</span><span style="padding-left: 20px" />
...
import { findDesByToc, toCAddDetail, tocUpd, tocDelTotal, tocDelDetail, searchWorkGroupWarehouse, tocScreenHead, tocScreenHeadNum, tocScreenDetail, tocScreenDetailNum, toCGenerateDes } from '@/api/user'
...
export default {
  ...
  watch: {
    viewed: function(viewed) {
      switch (viewed) {
        case true:
          this.labelTitle = '客户订单(toC)-未进行发运的记录'
          break
        case false:
          this.labelTitle = '客户订单(toC)-所有记录'
      }
    }
  },
  mounted: function() {
  ...
  methods: {
    ...
    searchDisOrder() {
      findDesByToc({ 'id': this.id }).then(res => {
    ...
    submitEdit() {
      ...
      if (this.checkSubmitAllFinish && this.checkSubmitTitle) {
        ...
        this.$confirm('提交所填客户订单信息, 是否确认修改?', '提示', {
          ...
          toCAddDetail(this.upEditNew)
          tocDelDetail(this.upEditDelete)
        }).catch(() => {
          this.$message({
            type: 'info',
            message: '已取消提交'
    ...
    toDespatch() {
      this.$confirm('是否确定将此toC客户订单生成发运订单?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        toCGenerateDes({ head: { id: this.warehouseId },
    ...
    toDespatchAll() {
      if (this.multipleSelection.length) {
        this.$confirm('是否确认将选中的客户订单生成发运订单?', '提示', {
          ...
          toCGenerateDes({ head: { id: this.warehouseId },
            details: this.multipleSelection }).then(res => {
            if (res.code === 200) {
    ...
    search() {
      ...
      this.labelTitle = '客户订单(toC)-查询结果'
      this.getList()
    },
    getListDetails() {
      this.tableLoading = true
      tocScreenDetail({ searchDTO: {
    ...
    deledeNowAll(index) {
      this.$confirm('是否确认删除此客户订单？（删除后无法撤销）', '提示', {
        ...
        tocDelTotal({ 'id': this.id }).then(response => {
    ...
    addNew() {
      this.$router.push('toccustomerlist')
    },
