<template>
  <div>
    <el-card style="border-radius: 20px; margin-top: 5px;" shadow="always">
      <div class="search">
        <el-form :inline="true" :model="form" label-width="120px">
          <el-form-item label-width="180px" label="入库计划-无计划单类型">
          ...
          <el-form-item label-width="180px" label="入库计划-无计划单状态">
    ...
    <div class="btngroups">
      ...
      <!-- <el-button size="small" icon="el-icon-newTask" type="success">生成分拣任务</el-button> -->
      ...
      <el-button size="small" type="danger" icon="el-icon-view" @click="scanHistory">{{ viewed ? "查看所有入库计划-无计划记录" : "返回" }}</el-button>
    </div>
    ...
    >
      ...
      <el-table-column :label="labelTitle" align="center">
        <el-table-column
          ...
          width="50"
        />
        <el-table-column align="center" label="入库计划-无计划单编号" :show-overflow-tooltip="true" prop="planId" width="120px" />
        <el-table-column align="center" sortable="custom" label="入库计划-无计划单类型" :show-overflow-tooltip="true" prop="inboundType" width="120px" :formatter="typeFormat" />
        <el-table-column align="center" label="入库批次" :show-overflow-tooltip="true" prop="inboundBatch" width="120px" />
        <el-table-column align="center" sortable="custom" prop="inboundState" label="入库计划-无计划单状态" width="120px" :formatter="stateFormat" />
        <el-table-column align="center" sortable="custom" label="仓库名称" :show-overflow-tooltip="true" prop="warehouseName" />
        <el-table-column align="center" label="来源类型" :show-overflow-tooltip="true" prop="sourceType" :formatter="sourceFormat" />
        ...
        <el-table-column align="center" sortable="custom" label="审核人" :show-overflow-tooltip="true" prop="checkPersonName" />
...
import { selectInboundPlan, auditPlan, screenContainer, receiveTable, screenGoods, delPlanDetail, selectInboundPlanNum, closeInboundPlan, cancelInboundPlan, addInboundBatch, selectInboundPlanDetailNum, selectInboundPlanDetail, screenExcelOutIPL, selectExcelOutIPL } from '@/api/user'
...
export default {
  ...
  watch: {
    receiptunorderState: function(newObj) {
      switch (newObj) {
        case 'true':
          this.viewed = false
          break
    ...
    viewed: function(newViewed) {
      switch (newViewed) {
        case true:
          this.labelTitle = '入库计划-无计划未完成记录'
          break
        case false:
          this.labelTitle = '入库计划-无计划所有记录'
      }
    }
  },
  created() {
    this.receiptunorderState = sessionStorage.getItem('receiptunorderState')
    if (this.receiptunorderState === 'true') {
      this.stateMin = null
      this.stateMax = null
      this.getList()
    } else {
  ...
  methods: {
    ...
    scanHistory: throttle(function() {
      ...
      if (this.viewed === false) {
        sessionStorage.setItem('receiptunorderState', true)

        this.stateMin = null
        this.stateMax = null
      } else {
        sessionStorage.setItem('receiptunorderState', false)
    ...
    rowDbclick(row) {
      ...
      this.$router.push('/unorderplantotal')
    },
    getSummaries(param) {
      const { columns } = param
      const sums = []
    ...
    getList() {
      ...
      stateMin: this.stateMin,
      stateMax: this.stateMax,
      startTime: this.value1[0], screen: this.screen, endTime: this.value1[1], startTime1: this.value2[0], endTime1: this.value2[1] }).then(response => {
    ...
    add() {
      this.$router.push('/unorderplanlist')
