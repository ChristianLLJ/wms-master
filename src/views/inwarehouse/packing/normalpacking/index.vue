<template>
  <div>
    ...
    <div class="btngroups">
      ...
      <el-button size="small" type="danger" icon="el-icon-view" @click="scanHistory">{{ viewed? "查看所有有计划-装箱记录" : "返回" }}</el-button>
    </div>

    <el-dialog title="请选择以下已质检的采购订单进行装箱" :visible.sync="dialogVisible" width="75%" class="showAll_dialog">
      <div style="height: 8%;">
        <el-row>
          <el-col :span="12">
    ...
    <el-dialog title="生成装箱任务单" :visible.sync="dialogPackVisible" width="75%" class="showAll_dialog2">
      <div class="cardContain">
        <div class="formContain">
          <el-form ref="packForm" :model="packForm" :rules="rules" :inline="true" class="demo-form-inline">
            <el-form-item label="装箱任务单号">
              <el-input v-model="packForm.packingId" size="mini" disabled style="width: 200px;" clearable />
            </el-form-item>
...
import { selectInPackage, screenContainer, selectInPackageNum, getPackHeadCode, selectInboundPlanDetailNum, selectInboundPlanDetail, closeInPackage, generateIPLToIPA, auditPack, screenGoods, delPackDetail, selectInPackageDetailNum, screenExcelOutIPA, selectExcelOutIPA, selectInPackageDetail, cancelInPackage } from '@/api/user'
import { searchsystemid, searchsku, searchallwarehouse } from '@/utils/search'
...
export default {
  ...
  data() {
    return {
      ...
      labelTable: '未进行装箱的采购订单',
      packageOptions: [],
      expands: [],
      labelTitle: '装箱任务-有计划未完成记录',
      currentPage: 1, // 第几页
  ...
  watch: {
    normalpackState: function(newObj) {
      switch (newObj) {
        case 'true':
          this.viewed = false
          break
    ...
    viewed: function(newViewed) {
      switch (newViewed) {
        case true:
          this.labelTitle = '装箱任务-有计划未完成记录'
          break
        case false:
          this.labelTitle = '装箱任务-有计划所有记录'
      }
    }
  },
  mounted: function() {
  ...
  created() {
    this.normalpackState = sessionStorage.getItem('normalpackState')
    if (this.normalpackState === 'true') {
      this.stateMin = null
      this.stateMax = null
      this.getList()
    } else {
  ...
  methods: {
    getPackHeadCode() {
      getPackHeadCode({ code: 'IPA' }).then(res => {
        if (res.code === 200) {
          this.packForm.packingId = res.data
        }
      })
    },
    ...
    getListDetail(n1, n2) {
      ...
      selectInboundPlanDetail({ searchDTO,
        pojo: { isSingle: true, isChecked: 1, isPackaged: false, isPlate: 0, skuName: this.skuForm.skuName, commodityId: this.skuForm.commodityId
        }}).then(res => {
    ...
    addChoose() {
      if (this.dataListSelections.length === 0) {
        ...
        this.getPackHeadCode()
        this.funPackage()
        this.formData.tablepack = this.dataListSelections
        this.tablepack1 = this.dataListSelections
        for (var j = 0; j < this.formData.tablepack.length; j++) {
          this.packTotalJudge += Number(this.formData.tablepack[j].num)
        }
      }
    },
    submitPack() {
              ...
              if (this.packTotalJudge !== this.packTotalJudge2) {
                ...
                this.$confirm('此操作将提交所填信息, 是否继续?', '提示', {
                  ...
                  generateIPLToIPA({ inboundPlanDetailHeadAndDetail: { details: this.tablepack1, head: {}}, packingDetailHeadAndDetail: { details: this.list, head: {
                    ...
                    packingId: this.packForm.packingId,
    ...
    scanHistory: throttle(function() {
      ...
      if (this.viewed === false) {
        sessionStorage.setItem('normalpackState', true)
        this.stateMin = null
        this.stateMax = null
      } else {
        sessionStorage.setItem('normalpackState', false)
        this.stateMin = 1
        this.stateMax = 3
      }
      this.getList()
    ...
    //       packingId: row.id
    //     }}).then(response => {
    //     this.formData.tablepack = response.data.list
    //   })
    // },
    rowDbclick(row) {
      sessionStorage.setItem('packChildId', row.id)
      sessionStorage.setItem('packingId', row.packingId)
      this.$router.push('/packingtotal')
    },
    deleteDetails(row) {
