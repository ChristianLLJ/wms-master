  ...
  <div>
    <div id="btnAndSearch" class="show_card1">
      <el-button-group>
        ...
        <el-tooltip class="item" effect="dark" content="重新填写客户订单，会重置所有已填写的数据" placement="bottom-start">
          <el-button type="danger" icon="el-icon-refresh" @click="refresh">重填</el-button>
        </el-tooltip>
        <el-tooltip class="item" effect="dark" content="提交填写好的客户订单" placement="bottom-start">
    ...
    <el-card class="show_card1" :body-style="{padding:'0px'}" shadow="always">
      <div slot="header" align="center">
        <span class="title">客户订单-C 商品信息</span><span style="padding-left: 20px" />
      ...
      <el-form ref="formData" :model="formData" :rules="formData.rules">
        ...
        >
          ...
          <el-table-column label="单价" prop="skuPrice" />
...
import { getHeadCode, tocAdd } from '@/api/user'
import { searchwarehouse, searchcustomer, searchreceiver, searchsku, searchsystemid } from '@/utils/search'
...
export default {
  ...
  methods: {
    ...
    goback() {
      this.$confirm('新增客户订单尚未提交，是否返回客户订单列表?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$router.push('customerorderC')
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '取消返回'
          ...
          this.$refs.formData.validate((valid) => {
            if (valid) {
              this.$confirm('提交所填客户订单信息, 是否确认?', '提示', {
                ...
                tocAdd({ head: {
                  'customerId': this.formTitle.data.customerId,
