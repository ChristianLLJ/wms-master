  ...
  <div>
    ...
    <el-card class="show_card1" :body-style="{padding:'20px'}">
      <div slot="header" class="clearfix">
        <span class="title">入库计划-有计划单头</span>
      </div>
    ...
    <el-card class="show_card1" :body-style="{padding:'0px'}">
      ...
      <el-form ref="formData" :model="formData" :rules="formData.rules">
        ...
        >
          ...
          <el-table-column v-if="false" label="是否装箱">
            <template slot-scope="scope">
              <span>
                <el-input v-model="scope.row.isPackaged" size="mini" />
              </span>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
              <el-button-group>
                ...
                <el-button v-if="show" size="mini" icon="el-icon-delete" type="danger" @click="delectTrue(scope.row, scope.$index)">删除</el-button>              </el-button-group>
            </template>
          </el-table-column>
        </el-table>
      </el-form>
...
<script>
import { addInboundPlan, selectInboundPlan, selectInboundPlanDetail, delPlanDetail, screenGoods, updPlanTable, getPlanHeadCode, selectPurchaseOrderDetail, selectPurchaseOrderDetailNum } from '@/api/user'
import { searchallwarehouse, searchcustomer, searchsupplier, searchsku, searchcon, searchsystemid } from '@/utils/search'
...
export default {
  ...
  methods: {
    ...
    changeOptions(row) {
      ...
      const item = this.supplierOptions.find(item1 => item1.customName === row.supplierName)
    ...
    funcCommodity(query) {
      setTimeout(() => {
        if (query !== '') {
          setTimeout(() => {
            this.commodityOptions = this.commodityOptions.filter(item => {
